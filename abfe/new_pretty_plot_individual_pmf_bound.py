...
fig1.tight_layout()
fig1.savefig(f'convergence-epoch-{nepochs}_individual_bound.png',dpi=1200)
fig1.show()
...
for i, stage in enumerate(['discharge','vanish']):
    ...
    mapper = plotPMF(f'convergence-epoch-{nepochs}_run-{run}-bound-{stage}_pmf.dat', ax, 'bound', stage)
# plot free on bottom row
#for i, stage in enumerate(['discharge','vanish']):
    #ax = fig2.add_subplot(2,2,i+3)
    #plotPMF(f'convergence-epoch-{nepochs}_run-{run}-free-{stage}_pmf.dat', ax, 'free', stage)
fig2.colorbar(mapper).set_label('Cumulative Sampling Time / ns')
fig2.tight_layout()
fig2.savefig(f'convergence-epoch-{nepochs}_individual_pmf_bound.png', dpi=1200)
fig2.show()
