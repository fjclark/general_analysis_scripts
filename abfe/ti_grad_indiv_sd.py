...
def plot(datafile, ax ,leg, stage):
    ...
    for i,l in enumerate(lines):
        ...
        elif l.startswith('#TI average gradients and standard deviation in kcal/mol'):
            for int in range(i+1, i+no_lam_vals+1):
    ...
    ax.plot(pmf_ti[:,0],pmf_ti[:,2])
    ax.set_xlabel(r'$\lambda$')
    ax.set_ylabel('Uncertainty in\n TI gradient')
    sbn.despine()
...
if __name__ == "__main__":
    ...
    for run in range(1, runs+1):
        ...
        fig.tight_layout()
        fig.savefig(f'ti_indiv_sd_{runs}_runs.png')